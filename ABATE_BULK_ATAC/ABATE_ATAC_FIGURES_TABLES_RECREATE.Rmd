...
dir_path <- "/Users/tybottorff/mounts/workspace/tbottorff/P576-1/saved_data/"
file_list <- list.files(path = dir_path, pattern = "^merged_mt_.*\\.txt$", full.names = TRUE)
...
upset_mt_var_data_2 <- mt_var_data %>%
  filter(p_value == 0.05, min_coverage == 20, min_var_freq == 0.003) %>%
...
count_data <- mt_var_data %>%
  filter(p_value %in% c(0.05), min_var_freq %in% c(0.003), min_coverage %in% c(20)) %>%
  ...
  group_by(min_coverage, p_value, min_var_freq, sort, donorId, responder, var_perc_category) %>%
  summarize(count = n(), .groups = "drop") %>%
  mutate(key = "A")
count_data_all <- count_data %>%
  group_by(min_coverage, p_value, min_var_freq, sort, donorId, responder) %>%
...
p_values_summary <- summed_data %>%
  group_by(min_var_freq, var_perc_category) %>%
  group_modify(~ {
