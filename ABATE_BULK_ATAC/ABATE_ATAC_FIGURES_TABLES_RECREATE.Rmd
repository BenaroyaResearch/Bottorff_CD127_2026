...
# Read in HOMER output, plot enrichment of transcription factor motifs in open chromatin between Tex subsets (figure 6C, supplementary data 3)
...
fig6C
ggsave("./figures/fig6C.pdf", fig6C, width = 6, height = 3)
write.csv(plot_df, "./saved_data/supp_data_3.csv", row.names = FALSE)

```

# Read in mitochondrial variant counts, plot quality control tests (supplementary figure 8)
...
hv2_start <- 57
hv2_end <- 372
supp_fig_8A <- ggplot(data = mt_var_data, aes(x = Position)) +
...
supp_fig_8A
ggsave("./figures/supp_fig_8A.pdf", supp_fig_8A, width = 5, height = 5)
...
supp_fig_8B <- upset(as.data.frame(upset_mt_var_data_2), sets = setdiff(colnames(upset_mt_var_data_2), c("sort", "Position", "most_freq_var_allele")), sets.bar.color = "#56B4E9", sets.x.label = "MT SNV counts", mainbar.y.label = "MT SNV sharing across donors", point.size = 1.5, show.numbers = FALSE, text.scale = 1)
supp_fig_8B
pdf("./figures/supp_fig_8B.pdf", height = 4.5, width = 5)
print(supp_fig_8B)
invisible(dev.off())
...
# Plot peak scores for open chromatin near cytotoxicity genes for all non-naive CD8s (supplementary figure 7A)
...
for (contrast in names(dge_grouped)[1]) {
  ...
  supp_fig_7A = Heatmap(tp, show_column_names = FALSE, show_row_names = TRUE, 
  ...
  plot(supp_fig_7A)
  pdf("./figures/supp_fig_7A.pdf", height = 7, width = 7.5)
  print(supp_fig_7A)
  invisible(dev.off())
}
