...
more_times <- read_xlsx("./raw_data/full_tn10_times.xlsx") %>%
  rename(`Masked ID of TN10 Clinical Data_Linsley_20190104_copy 20240716 JMP file` = MaskID,
         `Time.to.T1D (months)` = Time,
         T1D.event = Status)

metadata <- read_xlsx("./raw_data/t1d_metadata.xlsx") %>%
  select(-T1D.event, -`Time.to.T1D (months)`) %>%
  left_join(more_times, by = "Masked ID of TN10 Clinical Data_Linsley_20190104_copy 20240716 JMP file")
...
# adjust pvals
  mutate(p_adj = p.adjust(logrank_p, method = "BH"))
pvals
...
# Stratify participants by baseline frequencies of each cluster separately to compare T1D-free intervals (supplementary figure 2A)
...
run_one <- function(marker_col, cluster_idx){
  ...
  pdf(file = paste0("./figures/", "supp_fig_2B_", marker_col, ".pdf"), width = 3, height = 5)
  print(p)
  dev.off()
  # stats
...
# adjust pvals across 8 clusters
get_p <- function(sdiff){
  1 - pchisq(sdiff$chisq, df = length(sdiff$n) - 1)
}
p_top_raw <- map_dbl(results_list, ~ get_p(.x$top_vs_placebo))
p_bot_raw <- map_dbl(results_list, ~ get_p(.x$bot23_vs_placebo))
p_top_adj <- p.adjust(p_top_raw, method = "BH")
p_bot_adj <- p.adjust(p_bot_raw, method = "BH")
stats_df <- tibble(
  marker = marker_cols,
  p_top_vs_placebo_raw = p_top_raw,
  p_top_vs_placebo_adj = p_top_adj,
  p_bot_vs_placebo_raw = p_bot_raw,
  p_bot_vs_placebo_adj = p_bot_adj
)

```

# Stratify participants by 3-month frequencies of each cluster separately to compare T1D-free intervals (supplementary figure 2B)
...
# Stratify participants by 3-month frequencies of clusters 1-4 combined (Tex) to compare T1D-free intervals (supplementary figure X)
...
supp_fig_X
pdf("./figures/supp_fig_X.pdf", width = 3, height = 5)
print(supp_fig_X)
dev.off()
...
print(fig1D_left)
dev.off()
# p values comes from supplementary figure 2A (adjusted across 8 clusters)
...
pairwise_pvals
# p values comes from supplementary figure 2B (adjusted across 8 clusters)
...
# Stratify participants by 3-month frequencies of CD127+ (% Tex, figure 4B), CD57+ (% Tex, figure 4C), or PD-1+ (% Tex, figure 4D) to compare T1D-free intervals

```{r}

# doing all together here to adjust pvals across these 3 subsets
...
fig_4B
cd127_tex_pvals <- pairwise_survdiff(Surv(Time.to.Disease, T1D.event) ~ group, data = df_cd127, p.adjust.method = "none")
...
fig_4C
cd57_tex_pvals <-pairwise_survdiff(Surv(Time.to.Disease, T1D.event) ~ group, data = df_cd57, p.adjust.method = "none")
...
fig_4D
pd1_tex_pvals <- pairwise_survdiff(Surv(Time.to.Disease, T1D.event) ~ group, data = df_pd1, p.adjust.method = "none")
pdf("./figures/fig_4D.pdf", width = 3, height = 5)
print(fig_4D)
dev.off()
# adjust pvals
p_raw <- c(
  pd1_top  = pd1_tex_pvals$p.value["Placebo","Tep hi"],
  pd1_bot  = pd1_tex_pvals$p.value["Placebo","Tep lo"],
  cd57_top = cd57_tex_pvals$p.value["Placebo","Tep hi"],
  cd57_bot = cd57_tex_pvals$p.value["Placebo","Tep lo"],
  cd127_top = cd127_tex_pvals$p.value["Placebo","Tep hi"],
  cd127_bot = cd127_tex_pvals$p.value["Placebo","Tep lo"]
)
...
supp_fig_6B <- ggsurvplot(
  ...
  tables.theme = theme(
    ...
    theme(
      ...
      axis.title = element_text(size = 24)))
supp_fig_6B
pairwise_survdiff(Surv(Time.to.Disease, T1D.event) ~ group, data = df_cd127, p.adjust.method = "BH")
...
# Stratify participants by baseline frequencies of CD127+ (% Tex, supplementary figure 6C), CD57+ (% Tex, supplementary figure 6D), or PD-1+ (% Tex, supplementary figure 6E) to compare T1D-free intervals
...
# Plot baseline SKIM frequency vs. age (supplementary figure 5A)

```{r}

skim_by_age <- read_xlsx("./raw_data/skim_by_age.xlsx") %>%
  mutate(age_group = if_else(Age < 30, "<30", ">=30")) %>%
  # AbATE data doesn't add
  filter(Dataset == "TN10")
supp_fig_5A <- skim_by_age %>%
  ggplot(aes(
    x = Age,
...
supp_fig_5A
ggsave("./figures/supp_fig_5A.pdf", width = 5, height = 3)
# stats
# <30
...
# Stratify participants by age to compare T1D-free intervals (supplementary figure 5B)
...
supp_fig_5B
pdf("./figures/supp_fig_5B.pdf", width = 3, height = 5)
print(supp_fig_5B)
dev.off()
age_df_stats <- age_df %>%
...
pvals <- tibble(
  comparison = c("Placebo vs under 15", "Placebo vs 15 to 35", "Placebo vs over 35"),
  p_raw = c(p_young, p_middle, p_old)
) %>%
